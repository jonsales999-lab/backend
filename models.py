...
if not DATABASE_URL:
    ...
    # Lógica de correção para URLs do MySQL da Railway
    # O SQLAlchemy com mysql-connector-python exige o prefixo mysql+mysqlconnector://
    if DATABASE_URL.startswith("mysql://"):
        DATABASE_URL = DATABASE_URL.replace("mysql://", "mysql+mysqlconnector://", 1)
    # Caso a URL venha como postgres:// (comum em alguns deploys), corrigimos para postgresql://
    elif DATABASE_URL.startswith("postgres://"):
        DATABASE_URL = DATABASE_URL.replace("postgres://", "postgresql://", 1)

# Configuração do Engine
# pool_pre_ping=True ajuda a manter a conexão viva com o banco remoto (Railway)
db = create_engine(
    DATABASE_URL, 
    pool_pre_ping=True,
    pool_recycle=3600
